...
def factorize(
    ...
    if mode == "ws":
        ...
        for _ in range(n_iteration):
            ...
            t = sum(
                tuple(weights[i] * ws[i].T @ vs[i] for i in range(n_v)),
                axis=0,
            )

            b = sum(
                tuple(weights[i] * ws[i].T @ ws[i] @ h for i in range(n_v)),
                axis=0,
            )
        ...
        for _ in range(n_iteration):
            ...
            t = sum(
                tuple(weights[i] * vs[i] @ hs[i].T for i in range(n_v)),
                axis=0,
            )

            b = sum(
                tuple(weights[i] * w @ hs[i] @ hs[i].T for i in range(n_v)),
                axis=0,
            )
    ...
    plot_plotly(
        {
            "data": [
                ...
                for index, error_ in enumerate(error__)
            ],
            "layout": {
