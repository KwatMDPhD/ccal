...
if(length(packages)) install.packages(packages, dependencies=TRUE, repos='http://cran.us.r-project.org')
...
DISSECTOR_create_components.v1 <- function(
                                           ...
                                           theta = 0.5,                      # For method = nsNMF value of smoothing parameter theta
                                           prefix = "nMFComp",                 # Prefix for the component names
                                           gene_subset = "all-genes",        # Universe of genes to consider for matrix decomposition: "gene-sets", "all-genes"
                                           gene_sets_files = NULL,           # If gene_subset = "gene-sets" GMT files with gene sets
                                           gene_sets = NULL,                 # If gene_subset = "gene-sets" then name of the specific gene set(s) in gene_sets_file to use
                                           normalize_after_selection = T,    # If gene_subset = "gene-sets," normalize after selection the gene subset
...

{
    ...
    for (f in 1:length(feature.files)) {   # loop over feature types
        ...
        if (n.vals >= length(target.vec)*0.5) {    # Continuous value color map        
            image(1:Ns.1, 1:1, as.matrix(target.vec), zlim = c(0, max.cont.color), col=mycol[1: max.cont.color], axes=FALSE, main=main, sub = "", xlab= "",
                  ylab="", cex.main=2)
            #          image(1:Ns.1, 1:1, as.matrix(target.vec), zlim = range(target.vec), col=mycol, axes=FALSE, main=main, sub = "", xlab= "", ylab="")
        } else if (n.vals == 2) {  # binary
            image(1:Ns.1, 1:1, as.matrix(target.vec), zlim = range(target.vec), col=binary.col, axes=FALSE, main=main, sub = "", xlab= "", ylab="",
                  cex.main=2)
        } else {  # categorical
            image(1:Ns.1, 1:1, as.matrix(target.vec), zlim = range(target.vec), col=categ.col[1:n.vals], axes=FALSE, main=main, sub = "", xlab= "", ylab="",
                  cex.main=2)
        }
        ...
        axis(2, at=1:1, labels=target.name, adj= 0.5, tick=FALSE, las = 1, cex=1, cex.axis=cex.axis*character.scaling, line=0, font=2, family="",font.axis=1)
        axis(4, at=1:1, labels=paste(" IC       p-val"), adj= 0.5, tick=FALSE, las = 1, cex=1, cex.axis=cex.axis*character.scaling, font.axis=1, line=0, font=2, family="")
        ...
        for (i in 1:nrow(feature.mat)) {
            feature.vec <- feature.mat[i,]
            IC <- IC.v1(feature.vec, target.vec)
            IC <- signif(IC, 3)
        ...
        if (n.vals > 2) {    # Continuous value color map        
            ...
            image(1:dim(V)[2], 1:dim(V)[1], t(V), zlim = c(0, 1), col=binary.col, axes=FALSE, main="", cex.main=0.8,  sub = "", xlab= "", ylab="")
        }
                                             ...
                                             plot.dist.matrix        = F)
{
    ...
    categ.col2 <- c("#9DDDD6", # dusty green
                    "#FC8962", # red
                    "#9AC7EF", # sky blue
                    "#F970F9", # violet
                    "#FFE1DC", # clay
    ...
    if (plot.dist.matrix == T) {
        dist.matrix2 <- DISSECTOR_compute_assoc_or_dist.v1(input_matrix1 = dist.matrix, input_matrix2 = dist.matrix, object_type = "columns",
                                                           assoc_metric = "IC", distance = T)
        hc <- hclust(dist.matrix2, "ward")
        assoc.matrix.sorted <- dist.matrix[, hc$order]
        dist.matrix2 <- DISSECTOR_compute_assoc_or_dist.v1(input_matrix1 = dist.matrix, input_matrix2 = dist.matrix, object_type = "rows",
                                                           assoc_metric = "IC", distance = T)
    ...
    cols.V4 <- categ.col2[match(column.class, all.classes)]
    cex.cols <- 0.20 + 200/(ncol(V4) * max(nchar(colnames(V4))) + 200)
    mtext(colnames(V4), at=1:ncol(V4), side = 1, cex=cex.cols, col=cols.V4, line=0, las=3, font=2, family="")
    ...
    states <- matrix(0, nrow=k.classes, ncol=ncol(m.1))
    for (k in 1:k.classes) {
        states[k, x2[, "cutree.model"] == k] <- 1
    }

    all.features <- t(x2)
    all.features <- rbind(all.features, states)
    nodes2 <- paste("node_", seq(1, length(nodes)), "_", nodes, sep = "")
    all.features.names <- c(nodes2, "states", paste("state_", seq(1, k.classes), sep=""))
    all.features.descs <- c(nodes2, "states", paste("state_", seq(1, k.classes), sep=""))

    col.names <- paste(colnames(x2), collapse = "\t")
    col.names <- paste("SAMPLE", col.names, sep= "\t")
    write(noquote(col.names), file = membership.file, append = F, ncolumns = length(col.names))
    write.table(x2, file=membership.file, quote=F, col.names = F, row.names = T, append = T, sep="\t")

    # Define centroids

    centroids <- matrix(0, nrow=nrow(H), ncol=k.classes)
    for (i in 1:nrow(H)) {
    ...
    top_3_nodes <- matrix(NA, nrow=3, ncol=ncol(centroids))
    ...
    for (i in 1:ncol(H)) {
        ...
        print(paste(i, xp.local[i], yp.local[i]))
    ...
    Pxy <- array(0, dim= c(size.grid, size.grid, k.classes))
    for (k in 1:k.classes) {
    ...
    winning.class <- final.Pxy <- matrix(0, nrow=size.grid, ncol=size.grid)
    for (i in 1:size.grid) {
        for (j in 1:size.grid) {
            winning.class[i, j] <- which.max(Pxy[i, j, ])
    ...
    for (k in 1:3)  { # make plot 3 times (one with just domains, one with domains and points and the last one with samples names)
        ...
        for (i in 1:size.grid) {
            for (j in 1:size.grid) {
                x.p <- x.coor[i]
                y.p <- y.coor[j]
                col <- mycol.class[ceiling((myncolors - 1) * final.Pxy[i, j] + 1), winning.class[i, j]]
        ...
        if (k > 1) {         
            text(x.min + 0.05*x.len, 1.02*y.max, paste("Basic Layout and Samples (", length(xp), ")", sep=""), cex = 1.1, font=2, family="",
                 pos=4, col="darkred")
        } else {
    ...
    if (projection.set.norm == "wrt_train") {
        ...
        for (j in 1:nrow(H.projection)) H.projection[i,] <- (H.projection[i,] - min(H.projection[i,]))/(max(H.projection[i,]) - min(H.projection[i,]))
    }
    ...
    for (i in 1:ncol(H.projection)) {
        ...
        print(paste(i, xp.local.projection[i], yp.local.projection[i]))
    ...
    for (f in 1:length(feature.files)) {   # loop over feature types
        ...
        if (length(features[[f]]) == 1 & is.numeric(features[[f]])) {    # search for top matching features from that feature file 
            ...
            for (h in 1:nrow(m.2)) feat.IC[h] <- abs(IC.v1(m.2[h,], cutree.model))
        ...
        for (feat.n in 1:length(features[[f]])) { 
            ...
            #         IC <- IC.v1(feature.vec, cutree.model)
            #         null.IC <- vector(length=n.perm, mode="numeric")
            #         for (h in 1:n.perm) null.IC[h] <- IC.v1(feature.vec, sample(cutree.model))
            #         if (IC >= 0) {
            #             p.val <- sum(null.IC >= IC)/n.perm
            #         } else {
            #             p.val <- sum(null.IC <= IC)/n.perm
            ...
            text(x.min, 1.1*y.max + 0.03*y.len, paste(feature.name.short, " (",  names(features)[[f]], ")"), cex = 1.4, font=2, family="", pos=4, col="darkblue")
            ...
            for (i in 1:length(xp.projection)) {   # Missing value
                if (feature.vec[i] == max.cont.color) {
                    points(xp.projection[i], yp.projection[i], col="white", pch=4, cex=0.2)
                } else { # Present value
            ...
            if (produce.boxplots == T) {
                # par(mar = c(1, 1, 1, 1))         
                cutree.model3 <- cutree.model[locs]
                if (sd(feature.vec3) == 0) {
                    ...
                    feature.vec4 <-  y.max*1.05 - 0.02*y.len +
                        (y.len/7)*(feature.vec3 - min(feature.vec3))/(max(feature.vec3) - min(feature.vec3))
                }
                classes <- sort(unique(cutree.model3))
                boxplot(feature.vec4 ~ cutree.model3, main="", boxwex = 0.04*5/k.classes,
...
DISSECTOR_produce_mutation_file.v1 <- function(
                                               ...
                                               maf.mut.input_file,
                                               gct.output.file,
                                               variant.thres = 3,  # threhold in counts to define entries for variant classes (e.g. Missense_Mutation)
                                               change.thres = 3,  # threhold in counts to define entries for protein changes (e.g. p.S768I )
                                               genes_with_all_entries = NULL) # for this list of genes create entries for all protein changes (disregard threshold)
{
    ...
    for (g in 1:N.genes) {
        ...
        mut.table.row.names <- c(mut.table.row.names, paste(gene, "_MUT_All", sep=""))
        ...
        row.entry[locs3] <- 1
        mut.table <- rbind(mut.table, row.entry)
        mut.table.row.names <- c(mut.table.row.names, paste(gene, "_MUT_Nonsilent", sep=""))
        ...
        for (var in variant.classes.above.thres) {
            ...
            row.entry[locs3] <- 1
            mut.table <- rbind(mut.table, row.entry)
            mut.table.row.names <- c(mut.table.row.names, paste(gene, "_MUT_",   var, sep=""))
        ...
        for (change in prot.change.classes.above.thres) {
            ...
            row.entry[locs3] <- 1
            mut.table <- rbind(mut.table, row.entry)
            mut.table.row.names <- c(mut.table.row.names, paste(gene, "_MUT_",   change, sep=""))
